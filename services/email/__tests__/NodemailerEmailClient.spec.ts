...
import NodemailerEmailClient, { NodemailerConfig } from '../NodemailerEmailClient'
...
describe('NodemailerEmailClient', () => {
  ...
  describe('sendTemplatedEmail', () => {
    ...
    it('Should throw an error if the email created from the template does not contain neither a subject nor body',
      async () => {
        const params = {
          to: ['bar@bar.baz'],
          templateName: 'a template',
        ...
        await expect(async() => sut.sendTemplatedEmail(params)).rejects.toThrow(NoEmailBodyError)

        mockSESTemplateClient.createEmailFromTemplate.mockResolvedValueOnce({ text: 'A body' })
        await expect(async() => sut.sendTemplatedEmail(params)).rejects.toThrow(NoSubjectError)
    })
    ...
    it('Should throw an error if attempting to send an email from a non-whitelisted domain.', async() => {
      ...
      await expect(async() => sut.sendTemplatedEmail(params)).rejects.toThrow(InvalidEmailAddressesError)
    })
  })
})
